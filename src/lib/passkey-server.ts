...
export class PasskeyServer {
  ...
  async addPasskeyToExistingAccount(
    ...
    // Call registerPasskey with skipBlock to bypass the "Account already exists" check
    // User is authenticated, so we trust the request
    return this.registerPasskey(email, credentialData, challenge, {
      ...opts,
      skipBlock: true
    });
  }
  ...
  async registerPasskey(
    ...
    credentialData: any,
    challenge: string,
    opts?: { rpID?: string; origin?: string; skipBlock?: boolean }
  ) {
    // Block if account exists without passkeys (unless skipBlock is set for connect flows)
    if (!opts?.skipBlock && await this.shouldBlockPasskeyForEmail(email)) {
